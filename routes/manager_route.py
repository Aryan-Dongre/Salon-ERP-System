# This route contain a parts which is only related to manager
...
from models.appointment_module import update_appointment
...
def appointment():
    ...
    return render_template("appointment/manager_appointment.html",
                    appointments=appointments,
                    stats = stats,
                    staff_list = staff_list,
                    role_id=role_id)

@manager_bp.route("/appointment/update", methods=["POST"])
@login_required
...
def staff():
     ...
     return render_template(
                 "staff/manager_staff.html",
                 staff_list = staff_list,
                  stats = stats )

@manager_bp.route("/staff/update-specialization", methods=["POST"])
@login_required
...
@manager_bp.route("/staff/update-status", methods=["POST"])
@login_required
def update_status():
...
@manager_bp.route("/staff/remove", methods=["POST"])
@login_required
def delete_staff():
...
@manager_bp.route("/staff/update", methods=["POST"])
@login_required
...
@manager_bp.route("/staff/add", methods=["POST"])
@login_required
...
def customer():
    ...
    return render_template(
        "customer/manager_customer.html",
        customers=customers,
        card=card
    )

# Service Part 
@manager_bp.route("/service")
@login_required
def service():
